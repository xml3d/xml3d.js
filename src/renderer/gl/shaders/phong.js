XML3D.shaders.register("phong", {
    ...
    fragment : [
        ...
        "               vec3 shadowMapDir =  fragVertexPosition - pointLightPosition[i];",
        "               vec4 lspos = pointLightShadowMapCoord[i];",
        "			    vec3 abspos = abs(lspos);",
        "               float fs_z = -max(abspos.x, max(abspos.y, abspos.z));",
        "               vec4 clip = pointLightMatrix*vec4(0.0, 0.0, fs_z, 1.0);",
        "               float lsDepth = (clip.z / clip.w) * 0.5 + 0.5;",
        "//			    float lsDepth = max(abs(perspectiveDivPos.x),max(abs(perspectiveDivPos.y),abs(perspectiveDivPos.z)));",
        "			    vec3 lightuv = perspectiveDivPos.xyz;",
        "			    float depth = unpackDepth( textureCube(pointLightShadowMap[i], pointLightShadowMapCoord[i].xyz)) + pointLightShadowBias[i];",
        ...
        "			    vec3 perspectiveDivPos = abs(lspos.xyz / lspos.w);",
        "               float fs_z = -max(lspos.x, max(lspos.y, lspos.z));",
        "               vec4 clip = lspos*vec4(0.0, 0.0, fs_z, 1.0);",
        "               float lsDepth = (clip.z / clip.w) * 0.5 + 0.5;",
        "   ",
        "//             float depth = unpackDepth( textureCube(pointLightShadowMap[0], pointLightShadowMapCoord[0].xyz));",
        "//             vec4 lspos = pointLightShadowMapCoord[0];",
