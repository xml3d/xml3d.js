...
test("light matrices", 21, function () {
    ...
    var checkInit = frameLoaded.then(function (doc) {return doc.querySelector("xml3d")}).then(promiseOneSceneCompleteAndRendered).then(function (s) {
        ...
        var actual = new XML3D.math.mat4.create();
        var expected = new XML3D.math.mat4.create();

        var light = lights._lights[0];

        XML3D.math.mat4.translate(expected, expected, [0, 0, 10]);
        light.getWorldMatrix(actual);
        QUnit.closeArray(actual, expected, EPSILON, "RenderLight World matrix");

        XML3D.math.mat4.invert(expected, expected);
        var spot = lights._models.spot.lightModels[0];
        spot.getLightViewMatrix(actual);
        QUnit.closeArray(actual, expected, EPSILON, "LightViewMatrix");
        ...
        QUnit.closeArray(actual, expected, EPSILON, "LightProjectionMatrix");

        spot.getLightViewProjectionMatrix(actual);

        var params = {};
        lights.fillGlobalParameters(params, true);
        QUnit.closeArray(params.spotLightMatrix, actual, EPSILON, "spotLightMatrix");
    ...
    var setPosition = checkInit.then(changeFunction("setPosition")).then(promiseSceneRendered).then(function (s) {
        ...
        XML3D.math.mat4.translate(expected, expected, [0, 0, 10]);
        light.getWorldMatrix(actual);
        QUnit.closeArray(actual, expected, EPSILON, "setPosition::RenderLight World matrix");

        XML3D.math.mat4.translate(expected, expected, [1, 1, 1]);
        XML3D.math.mat4.invert(expected, expected);
        var spot = lights._models.spot.lightModels[0];
        spot.getLightViewMatrix(actual);
        QUnit.closeArray(actual, expected, EPSILON, "setPosition::LightViewMatrix");

        // Depends on how the near/far plane is calculated. Thus this is just for regression:
        expected = [2.4142074584960938, 0, 0, 0, 0, 2.4142074584960938, 0, 0, 0, 0, -1.1847789287567139, -1, 0, 0, -2.184778928756714, 0 ];
        spot.getLightProjectionMatrix(actual);
        QUnit.closeArray(actual, expected, EPSILON, "setPosition::LightProjectionMatrix");

        spot.getLightViewProjectionMatrix(actual);

        var params = {};
        lights.fillGlobalParameters(params, true);
        QUnit.closeArray(params.spotLightMatrix, actual, EPSILON, "setPosition::spotLightMatrix");
    ...
    var setDirection = setPosition.then(changeFunction("setDirection")).then(promiseSceneRendered).then(function (s) {
        ...
        XML3D.math.mat4.translate(expected, expected, [0, 0, 10]);
        light.getWorldMatrix(actual);
        QUnit.closeArray(actual, expected, EPSILON, "setDirection::RenderLight World matrix");

        XML3D.math.mat4.translate(expected, expected, [1, 1, 1]);
        XML3D.math.mat4.rotateX(expected, expected, -0.1);
        XML3D.math.mat4.invert(expected, expected);
        var spot = lights._models.spot.lightModels[0];
        spot.getLightViewMatrix(actual);
        QUnit.closeArray(actual, expected, EPSILON, "setDirection::LightViewMatrix");

        // Depends on how the near/far plane is calculated. Thus this is just for regression:
        expected = [2.4142074584960938, 0, 0, 0, 0, 2.4142074584960938, 0, 0, 0, 0, -1.1646840572357178, -1, 0, 0, -2.1646840572357178, 0 ];
        spot.getLightProjectionMatrix(actual);
        QUnit.closeArray(actual, expected, EPSILON, "setDirection::LightProjectionMatrix");

        spot.getLightViewProjectionMatrix(actual);

        var params = {};
        lights.fillGlobalParameters(params, true);
        QUnit.closeArray(params.spotLightMatrix, actual, EPSILON, "setDirection::spotLightMatrix");
