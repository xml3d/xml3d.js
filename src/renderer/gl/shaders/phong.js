XML3D.shaders.register("phong", {
    ...
    vertex : [
        ...
        //"uniform mat4 pointLightProjection[ MAX_POINTLIGHTS ];",
        //"   varying vec4 pointLightShadowMapCoord[MAX_POINTLIGHTS];",
        //"#endif",     //shadowmapcoords calculation moved to fragmentshader so no longer needed
        ...
        //"       pointLightShadowMapCoord[i] = pointLightMatrix[i] *vec4(worldPosition, 1.0);",
        //"    }",
        //"#endif",                 //no longer needed -> calculation needed to move to fragmentshader
        "#if MAX_SPOTLIGHTS > 0 && HAS_SPOTLIGHT_SHADOWMAPS",
        "    for(int i = 0; i < MAX_SPOTLIGHTS; i++) {",
        "      spotLightShadowMapCoord[i] = spotLightMatrix[i] * vec4(worldPosition, 1.0);",
        "    }",
        "#endif",
        "#if MAX_DIRECTIONALLIGHTS > 0 && HAS_DIRECTIONALLIGHT_SHADOWMAPS",
        "    for(int i = 0; i < MAX_DIRECTIONALLIGHTS; i++) {",
        "      directionalLightShadowMapCoord[i] = directionalLightMatrix[i] * vec4(worldPosition, 1.0);",
        "    }",
        "#endif",
        "}"
    ].join("\n"),
    ...
    fragment : [
        ...
        "uniform bool pointLightCastShadow[MAX_POINTLIGHTS];",
            ...
            "uniform mat4 pointLightProjection[MAX_POINTLIGHTS];",
        //"   varying vec4 pointLightShadowMapCoord[MAX_POINTLIGHTS];", //no longer needed
            "#endif",
        "#endif",
        ...
        "               vec3 shadowMapDir =  fragWorldPosition - pointLightPosition[i];",

        "               vec3 abs = abs(shadowMapDir);",
        "               vec3 vTmp = shadowMapDir;",
        "               float maxComp = max(abs.x, max(abs.y, abs.z));",
        "               if(maxComp == shadowMapDir.x){",
        "                   float tmp = vTmp.z;",
        "                   vTmp.z = -vTmp.x;",
        "                   vTmp.x = tmp;",
        "               }",
        ...
        "                   vTmp.y = tmp;",
        "               }",
        "               if(maxComp == shadowMapDir.z){",
        "                   vTmp.z = -vTmp.z;",
        "               }",
        "               float lsDepth = length(shadowMapDir);",
        "               vec4 dvec  = pointLightProjection[i] * vec4(vTmp ,1.0);",
        "               lsDepth = dvec.z / dvec.w *0.5+0.5;",
        "			    float depth = unpackDepth( textureCube(pointLightShadowMap[i], shadowMapDir)) +  pointLightShadowBias[i];",
