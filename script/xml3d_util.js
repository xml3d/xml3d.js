...
org.xml3d.debug = {
	...
	doLog : function(msg, logType) {
		...
		if (org.xml3d.debug.numLinesLogged === org.xml3d.debug.maxLinesToLog) {
			msg = "Maximum number of log lines (=" + org.xml3d.debug.maxLinesToLog
 ...
 * @param inode node id of OrientationInterpolator element in X3D namespace
 * @param tnode node id of transform element in Xml3D namespace
 * @param tfield target field of interpolation, must be 'rotation' or 'scaleOrientation'
 */
org.xml3d.util.X3DOrientationInterpolation = 
function(inode, tnode, tfield)
{
	this.inodeId = inode;
	this.tnodeId = tnode;
	this.tnode = null;
...
org.xml3d.util.X3DOrientationInterpolation.prototype.isValid = function()
{
	if (!this.isInit)
		this.initialize();
	return this.valid;
...
org.xml3d.util.X3DOrientationInterpolation.prototype.initialize = function()
{
	this.isInit = true;
	this.valid = false;
	
	var inode = org.xml3d.util.X3DOrientationInterpolation.getElementById(this.inodeId, org.xml3d.x3dNS);
	if (!inode)
	{
		org.xml3d.debug.logError("Can't find X3D node: " + this.inodeId);
		return;
	}
	this.tnode = org.xml3d.util.X3DOrientationInterpolation.getElementById(this.tnodeId, org.xml3d.xml3dNS);
	if (!this.tnode)
	{
...
org.xml3d.util.X3DOrientationInterpolation.prototype.set_fraction = function(t)
{
	var value = this.interpolate(t, function(a, b, t) {
		return a.slerp(b, t).toAxisAngle();
	});
	this.tnode[this.tfield].x = value[0];
	this.tnode[this.tfield].y = value[1];
	this.tnode[this.tfield].z = value[2];
	this.tnode[this.tfield].angle = value[3];
};
