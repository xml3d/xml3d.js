...
(function(webgl) {
	...
	var OctreeDataAdapter = function(parent) {
		...
		webgl.OctreeComposer.call(this);
	
		if (parent)
			if (webgl.OctreeNode.REASONS_TO_SPLIT.BIG_DATA in parent.reasonsToSplit)
	...
	XML3D.createClass(OctreeDataAdapter, XML3D.webgl.OctreeComposer);
	...
	OctreeDataAdapter.prototype.setVolumeData = function(volumeDataArray, volumeGridSize) {
		...
		this.fillOctreeWithData( { volumeDataArray: volumeDataArray,
								   volumeGridSize : volumeGridSize } );
	...
	OctreeDataAdapter.prototype.updateOctreeLevel = function() {
		if (this.parent)
			if (webgl.OctreeNode.REASONS_TO_SPLIT.BIG_DATA in this.parent.reasonsToSplit)
				this.octreeLevelArray[0] = 1;
			else 
				this.octreeLevelArray[0] = this.parent.octreeLevelArray[0] * 2;
		else
			this.octreeLevelArray[0] = 1;
	};
	
	OctreeDataAdapter.prototype.getMaxGridSizeWithOverlaps = function(trueGridSize) {
		return [ Math.ceil(trueGridSize[0]+2),
				 Math.ceil(trueGridSize[1]+2),
				 Math.ceil(trueGridSize[2]+2)];
	};
	...
	OctreeDataAdapter.prototype.generateChildrenIfDataDoesNotFitOneTexture = function(trueGridSize) {
		...
		var maxTexSize = XML3D.webgl.MAX_TEXTURE_SIZE * XML3D.webgl.MAX_TEXTURE_SIZE;
		var dataValsCnt = texture2DWidth * texture2DHeight;

		if (dataValsCnt > maxTexSize) {
			this.needChildren(webgl.OctreeNode.REASONS_TO_SPLIT.BIG_DATA);
			...
			if (this.hasActiveChildren()) {
				...
				this.tryDeactivateChildren(webgl.OctreeNode.REASONS_TO_SPLIT.BIG_DATA);
			}
		...
		for (var j=0; j<3; j++) {
			...
			if (posClose > 0) {
				...
				if (decimalPart < 0.5)  // need previous pixel
					newPosClose = Math.floor(posClose - 1);
				else
	...
	OctreeDataAdapter.prototype.create2DTextureFromData = function(volumeData) {
		...
		this._assignMosaicFactor(new Float32Array([ xFactor, xFactor ]));
			...
			if (this.trueGridPosition) {
				...
				this._getIndexToRead = function(i, x, y, z) {
					return (x+gridPosition[0]) + 
						   (y+gridPosition[1])*(parentGridSize[0]) + 
						   (z+gridPosition[2])*(parentGridSize[0]*parentGridSize[1]);
				};
			}
			else
		...
		var options = { wrapS : gl.CLAMP_TO_EDGE,
		...
		ctex.gridStep      = new Float32Array([1/(this.mosaicFactorArray[0]*gridSize[0]),
											   1/(this.mosaicFactorArray[1]*gridSize[1])]);
    ...
    webgl.OctreeDataAdapter = OctreeDataAdapter;
